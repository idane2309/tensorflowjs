...
async function train() {
  ...
  // Classify hand gestures corresponding to rock, paper, scissors, lizard, and spock. 
  model = tf.sequential({
    layers: [
        tf.layers.flatten({inputShape: mobilenet.outputs[0].shape.slice(1)}),
        tf.layers.dense({units: 100, activation: 'relu'}),
        tf.layers.dense({units: 100, activation: 'relu'}),
        tf.layers.dense({units: 100, activiation: 'relu'}),
        tf.layers.dense({units: 5, activtion: 'softmax'})
    ]
  });
